...
const TIMELOCK_ELAPSED_MS = 6 * 60 * 60 * 1000; // 6 hours already elapsed

function CountdownTimer({ delayMs, elapsedAtMount }: { delayMs: number; elapsedAtMount: number }) {
  const [elapsed, setElapsed] = useState(elapsedAtMount);
  useEffect(() => {
...
const PENDING_TIMELOCKS = [
  { id: "0xabc", action: "Update USDY daily limit: 1000 → 2000" },
];
        ...
        <div>
          ...
          {PENDING_TIMELOCKS.map((action) => (
            ...
            >
              ...
              <CountdownTimer delayMs={TIMELOCK_DELAY_MS} elapsedAtMount={TIMELOCK_ELAPSED_MS} />
