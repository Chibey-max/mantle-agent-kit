  ...
  return (
    ...
    >
      ...
      <div className={`max-w-[80%] ${isAgent ? "" : "items-end"} flex flex-col gap-1`}>
        ...
        <div
          className="text-[10px] text-[var(--color-text-muted)] px-1"
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        // Build history from current messages (exclude the welcome message id="0")
      ...
      <div className="px-4 pb-2 flex gap-2 overflow-x-auto scrollbar-hide">
        {QUICK_COMMANDS.map((cmd) => (
          <button
            ...
            disabled={isTyping}
            className="flex-shrink-0 text-xs px-3 py-1.5 rounded-full transition-all hover:scale-105 hover:border-[rgba(0,212,170,0.3)] disabled:opacity-50 glass-pill"
            style={{
