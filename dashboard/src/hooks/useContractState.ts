...
export function useAgentIdentity(identityAddress?: `0x${string}`, agentAddress?: `0x${string}`) {
  ...
  // Guard against BigInt overflow — realistic token IDs never exceed 10M
  const MAX_TOKEN_ID = BigInt(10_000_000);
  const rawBigInt = tokenIdRaw as bigint | undefined;
  const tokenId = rawBigInt !== undefined
    ? (rawBigInt > MAX_TOKEN_ID ? 0 : Number(rawBigInt))
    : undefined;
